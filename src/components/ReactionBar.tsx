...
import { REACTIONS, hottest, colorClass, isLight, type Reaction, type SocialPostRow, type SocialColor } from '../lib/social'
...
export function ReactionBar({ row, color, size, onReact }: {
  ...
  // Blue/red cards are dark — keep non-hot button text light so the count stays legible.
        ...
        return (
          <motion.button
            ...
            className={`flex-1 flex items-center justify-center gap-1 border-[3px] border-ink
              ...
              ${isHot ? 'bg-ink text-paper' : `${tint} ${restText}`}`}
          >
            <span>{r.emoji}</span>
